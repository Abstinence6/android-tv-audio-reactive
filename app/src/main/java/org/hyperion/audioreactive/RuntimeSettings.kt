...
data class MqttBrokerSettings(val ip: String, val port: Int, val username: String, val password: String) {
    fun valid() = validPrivateIpv4(ip) && port in 1..65535
    fun publicState() = "$ip:$port"
    ...
    companion object {
        ...
        private fun validPrivateIpv4(value: String): Boolean {
            ...
            if (octets.size != 4 || octets.any { it.isEmpty() || it.length > 3 || !it.all(Char::isDigit) }) return false
            val numbers = octets.map { it.toIntOrNull() ?: return false }
            if (octets.zip(numbers).any { (raw, number) -> raw != number.toString() }) return false
            return numbers[0] == 10 || numbers[0] == 172 && numbers[1] in 16..31 || numbers[0] == 192 && numbers[1] == 168
        }
    }
...
object MqttClientPolicy {
    fun uri(settings: MqttBrokerSettings): String {
        require(settings.valid()) { "MQTT broker must be a private RFC1918 IPv4 endpoint with a valid port." }
        return "tcp://${settings.ip}:${settings.port}"
    }

    fun validUri(uri: String): Boolean {
        val match = Regex("tcp://([0-9.]+):([0-9]{1,5})").matchEntire(uri) ?: return false
        return MqttBrokerSettings.fromInput(match.groupValues[1], match.groupValues[2], "", "").settings != null
    }

    fun options(settings: MqttBrokerSettings): MqttClientOptions {
        require(settings.valid()) { "MQTT broker settings are invalid." }
        return if (settings.username.isBlank()) MqttClientOptions(null, null) else MqttClientOptions(settings.username, settings.password.toCharArray())
    }
}

enum class RenderMode { AUDIO, VIDEO, VIDEO_AUDIO, ANIMATION }
/** Playback remains the safe default; microphone is selectable only while a real input device is present. */
enum class AudioInput { PLAYBACK, MICROPHONE }
...
data class AudioSettings(
    ...
    val videoAudioEffect: VideoAudioEffect = VideoAudioEffect.BEAT_PULSE,
    ...
    /** VIDEO_AUDIO only: retain video on silence at this bounded brightness. Zero preserves legacy blackout. */
    val videoAudioSilenceBrightnessFloor: Float = 0f,
    /** VIDEO_AUDIO only: stable silence must persist before fade-down begins. */
    val silenceHoldMillis: Int = 500,
    /** VIDEO_AUDIO only: fade duration for both silence and recovery. */
    val silenceFadeMillis: Int = 500,
    /** VIDEO_AUDIO only: smooth silence fade is opt-in from the visible local control. */
    val silenceFadeEnabled: Boolean = false,
    val audioInput: AudioInput = AudioInput.PLAYBACK,
) {
    ...
    /** Stable full-display frame shape lets active AUDIO/VIDEO/VIDEO_AUDIO transitions keep one router. */
    fun liveCaptureFrame() = SourceFrameSpec(videoQuality.width, videoQuality.height, fps)
    fun valid(): Boolean = brightness in 0f..1f && sensitivity in .25f..3.25f && fps in CaptureCadence.MIN_FPS..CaptureCadence.MAX_FPS &&
        ...
        effectParameters.valid() && VideoSaturationPolicy.valid(videoSaturationPercent) && mqttBroker.valid() && videoAudioSilenceBrightnessFloor in 0f..brightness && silenceHoldMillis in 0..3_000 && silenceFadeMillis in 100..2_000 && wledCalibrations.size <= 64 && wledCalibrations.all(WledScreenCalibration::validFor) && wledCalibrations.map { it.identity }.distinct().size == wledCalibrations.size
...
class SharedPreferencesAudioSettingsStore(context: Context): AudioSettingsStore {
 ...
 override fun load():AudioSettings?=try { val effect=preferences.getString(EFFECT,null)?.let { n->Effect.entries.firstOrNull { it.name==n } }?:return null; val output=OutputMode.entries.firstOrNull { it.name==preferences.getString(MODE,null) }?:OutputModeMigration.fromLegacyIds(preferences.getString(OUTPUTS,null)); val mode=RenderMode.entries.firstOrNull { it.name==preferences.getString(RENDER_MODE,null) }?:RenderMode.AUDIO; val unifiedFps=preferences.getInt(FPS,preferences.getInt(VIDEO_FPS,20)); val wled=decodeWled(preferences.getString(WLED_DEVICES,"")); val hyperion=decodeHyperion(preferences.getString(HYPERION_DEVICES,"")); val broker=MqttBrokerSettings.fromInput(preferences.getString(MQTT_IP,MqttBrokerSettings.defaults().ip).orEmpty(),preferences.getInt(MQTT_PORT,MqttBrokerSettings.defaults().port).toString(),preferences.getString(MQTT_USERNAME,"").orEmpty(),preferences.getString(MQTT_PASSWORD,"").orEmpty()).settings?:return null; AudioSettings(effect,preferences.getFloat(BRIGHTNESS,Float.NaN),preferences.getFloat(SENSITIVITY,Float.NaN),unifiedFps,output,wled,preferences.getStringSet(WLED_SELECTED,emptySet())?.toSet()?:emptySet(),hyperion,preferences.getString(HYPERION_SELECTED,null),mode,VideoQuality.safe(preferences.getString(VIDEO_QUALITY,null)),unifiedFps,preferences.getFloat(AUDIO_BOOST,.35f),preferences.getInt(WLED_ZONES,128).coerceIn(16,512).let { it-it%16 },decodeEffectParameters(preferences.getString(EFFECT_PARAMS,null)),decodeCalibrations(preferences.getString(WLED_CALIBRATIONS,"")),VideoEffect.entries.firstOrNull{it.name==preferences.getString(VIDEO_EFFECT,null)}?:VideoEffect.NORMAL,VideoAudioEffectCatalogue.fromPersistedName(preferences.getString(VIDEO_AUDIO_EFFECT,null))?:VideoAudioEffect.BEAT_PULSE,AnimationEffect.entries.firstOrNull{it.name==preferences.getString(ANIMATION_EFFECT,null)}?:AnimationEffect.WATER,AnimationColour.entries.firstOrNull{it.name==preferences.getString(ANIMATION_COLOUR,null)}?:AnimationColour.AUTO,preferences.getInt(VIDEO_SATURATION,VideoSaturationPolicy.DEFAULT_PERCENT),broker,preferences.getFloat(VIDEO_AUDIO_SILENCE_FLOOR,0f),preferences.getInt(SILENCE_HOLD_MILLIS,500),preferences.getInt(SILENCE_FADE_MILLIS,500),preferences.getBoolean(SILENCE_FADE_ENABLED,false),AudioInput.entries.firstOrNull{it.name==preferences.getString(AUDIO_INPUT,null)}?:AudioInput.PLAYBACK).takeIf(AudioSettings::valid) } catch(_:Exception){null}
 override fun save(s:AudioSettings) { require(s.valid()); val videoAudioEffect=VideoAudioEffectCatalogue.pickerEffect(s.videoAudioEffect); check(preferences.edit().putString(EFFECT,s.effect.name).putFloat(BRIGHTNESS,s.brightness).putFloat(SENSITIVITY,s.sensitivity).putInt(FPS,s.fps).putString(MODE,s.outputMode.name).putString(RENDER_MODE,s.renderMode.name).putString(VIDEO_QUALITY,s.videoQuality.name).putString(VIDEO_EFFECT,s.videoEffect.name).putString(VIDEO_AUDIO_EFFECT,videoAudioEffect.name).putString(ANIMATION_EFFECT,s.animationEffect.name).putString(ANIMATION_COLOUR,s.animationColour.name).putInt(VIDEO_SATURATION,s.videoSaturationPercent).putString(MQTT_IP,s.mqttBroker.ip).putInt(MQTT_PORT,s.mqttBroker.port).putString(MQTT_USERNAME,s.mqttBroker.username).putString(MQTT_PASSWORD,s.mqttBroker.password).putFloat(VIDEO_AUDIO_SILENCE_FLOOR,s.videoAudioSilenceBrightnessFloor).putInt(SILENCE_HOLD_MILLIS,s.silenceHoldMillis).putInt(SILENCE_FADE_MILLIS,s.silenceFadeMillis).putBoolean(SILENCE_FADE_ENABLED,s.silenceFadeEnabled).putString(AUDIO_INPUT,s.audioInput.name).remove(VIDEO_FPS).putFloat(AUDIO_BOOST,s.audioBoost).putInt(WLED_ZONES,s.wledSourceZones).putString(EFFECT_PARAMS,encodeEffectParameters(s.effectParameters)).putString(WLED_CALIBRATIONS,encodeCalibrations(s.wledCalibrations)).putString(WLED_DEVICES,encodeWled(s.wledDevices)).putStringSet(WLED_SELECTED,s.selectedWledIdentities).putString(HYPERION_DEVICES,encodeHyperion(s.hyperionDevices)).apply { if(s.selectedHyperionIdentity==null) remove(HYPERION_SELECTED) else putString(HYPERION_SELECTED,s.selectedHyperionIdentity) }.remove(OUTPUTS).commit()) }
 private fun encoded(fields:List<String>)=fields.joinToString("|"){Base64.getUrlEncoder().encodeToString(it.encodeToByteArray())}
 private fun decoded(item:String)=item.split('|').mapNotNull { runCatching { Base64.getUrlDecoder().decode(it).decodeToString() }.getOrNull() }
...
object RuntimeSettings {
    ...
    @Synchronized fun initialize(s: AudioSettingsStore) { store = s; current = s.load()?.let(::canonicalize)?.takeIf { it.valid() } ?: AudioSettings.defaults() }
    ...
    private fun replaceLocked(transform: (AudioSettings) -> AudioSettings): AudioSettings {
        ...
        synchronized(this) {
            next = canonicalize(transform(current))
            require(next.valid())
            store.save(next)
            current = next
    ...
    private fun canonicalize(settings: AudioSettings) = settings.copy(videoAudioEffect = VideoAudioEffectCatalogue.pickerEffect(settings.videoAudioEffect))
}
