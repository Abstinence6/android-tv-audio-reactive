...
data class AudioSettings(
    ...
    fun valid(): Boolean = brightness in 0f..1f && sensitivity in .25f..3.25f && fps in CaptureCadence.MIN_FPS..CaptureCadence.MAX_FPS &&
        ...
        effectParameters.valid() && VideoSaturationPolicy.valid(videoSaturationPercent) && mqttBroker.valid() && wledCalibrations.size <= 64 && wledCalibrations.all(WledScreenCalibration::validFor) && wledCalibrations.map { it.identity }.distinct().size == wledCalibrations.size
