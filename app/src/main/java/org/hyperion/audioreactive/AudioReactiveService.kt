...
class AudioReactiveService : Service() {
 ...
 override fun onStartCommand(intent:Intent?,flags:Int,startId:Int):Int {
  ...
  if(!admission.reserve(ids)){broadcastAdmissionFailed(generation);return START_NOT_STICKY}
 ...
 private fun audioLoop(initial:AudioSettings){val samples=ShortArray(CaptureCadence.ANALYSIS_SAMPLES);val a=PcmAnalyzer();val r=EffectFrameRenderer(initial.captureFrame().width);val smoother=RgbFrameSmoother(initial.captureFrame().bytes);var tick=0L;val began=System.nanoTime();while(running.get()){val now=System.nanoTime();val n=recorder!!.read(samples,0,samples.size,AudioRecord.READ_BLOCKING);if(n<=0)break;val s=LiveRendererSettings.apply(initial);val f=a.analyze(samples,n,s.sensitivity);val raw=r.render(s.effect,f,s.brightness,tick++,s.effectParameters);if(!sendFrame(smoother.apply(raw,!f.signalPresent||s.brightness==0f),f,began,tick))break;sleep(now,initial.fps)};stop()}
 private fun videoLoop(initial:AudioSettings){val processor=VideoFrameProcessor(initial.captureFrame().width,initial.captureFrame().height);val smoother=RgbFrameSmoother(initial.captureFrame().bytes);val samples=ShortArray(CaptureCadence.ANALYSIS_SAMPLES);val analyzer=if(initial.requiresAudio())PcmAnalyzer()else null;var features:AudioFeatures?=null;var frames=0L;val began=System.nanoTime();while(running.get()){val started=System.nanoTime();val live=LiveRendererSettings.apply(initial);if(analyzer!=null){val n=recorder!!.read(samples,0,samples.size,AudioRecord.READ_NON_BLOCKING);if(n>0)features=analyzer.analyze(samples,n,live.sensitivity)};val image=reader!!.acquireLatestImage();if(image!=null)try{if(processor.copyImage(image)){frames++;val raw=processor.compose(features,live);if(!sendFrame(smoother.apply(raw,live.brightness==0f),features,began,frames))break}else{status=CaptureStatus.VIDEO_UNAVAILABLE_OR_PROTECTED;VideoCaptureFailurePolicy.blackoutAndTerminate{router?.stop();router=null};broadcast();break}}finally{image.close()};sleep(started,initial.fps)};stop()}
