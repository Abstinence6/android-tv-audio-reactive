...
class AudioReactiveService : Service() {
 ...
 override fun onStartCommand(intent:Intent?,flags:Int,startId:Int):Int {
  ...
  if(!lifecycle.whileStarting { channel(); foregroundTypesRequested=foregroundTypes; startForeground(ID,notification(animation),foregroundTypes) }) { admission.discardPending(); return START_NOT_STICKY }
 ...
 private fun rejectInvalidStart(generation:Long){invalidAdmissionGeneration=generation;terminalStop(TerminalCause.EXPLICIT_STOP)}
 private fun start(data:Intent,s:AudioSettings,generation:Long){
  try {
   ...
   if(!lifecycle.activate {
    alive=true; admittedSettings=s; requestedRender=s.renderMode; committedRender=s.renderMode; transitions=LocalTransitionPolicy(s.renderMode); LiveRendererSettings.begin(s); broadcast(generation); running.set(true); router!!.start()
   ...
   status=CaptureStatus.ROUTER_INIT_FAILED;LocalStatusStore.reset(status);broadcastAdmissionFailed(generation);broadcast();terminalStop(TerminalCause.TRANSITION_EXCEPTION)
  }
 }
 private fun startAnimation(s:AudioSettings,generation:Long){
  try {
   ...
   if(!lifecycle.acquire(acquire={ admission.consume { ids -> OutputRouter.create(s,ids.wled,ids.hyperion) } },release={it.stop()},assign={router=it})) return
   if(!lifecycle.activate { alive=true; admittedSettings=s; requestedRender=s.renderMode; committedRender=s.renderMode; transitions=LocalTransitionPolicy(s.renderMode); LiveRendererSettings.begin(s); broadcast(generation); running.set(true); router!!.start(); status=CaptureStatus.CAPTURE_ACTIVE_ANIMATION; LocalStatusStore.reset(status); broadcast() }) return
   videoLoop(s)
  } catch(failure:Exception) { Log.w(TAG, "$STARTUP_FAILURE_DIAGNOSTIC (${failure.javaClass.simpleName})"); status=CaptureStatus.ROUTER_INIT_FAILED; LocalStatusStore.reset(status); broadcastAdmissionFailed(generation); broadcast(); terminalStop(TerminalCause.TRANSITION_EXCEPTION) }
 }
 ...
 private fun transition(intent:Intent) {
  ...
  lifecycle.whileActive {
   ...
   try {
    ...
    requestedRender=target; transitionEpoch=epoch
    ...
    terminalStop(TerminalCause.TRANSITION_EXCEPTION) { status=CaptureStatus.ROUTER_INIT_FAILED; router?.stop() }
   }
  }
 }
 private fun setForegroundTypesFor(needs:RenderRequirements) {
  ...
  foregroundTypesRequested=types; startForeground(ID,notification(!needs.audio&&!needs.video),types)
 }
 private fun projectionCallback(owned:MediaProjection)=object:MediaProjection.Callback(){override fun onStop(){
  lifecycle.onProjectionStopped(owned) { terminalStop(TerminalCause.UNEXPECTED_PROJECTION_REVOKE) }
 }}
 private fun reconcileSources(p:MediaProjection,s:AudioSettings):Boolean = lifecycle.whileActive {
  if(s.requiresAudio()&&recorder==null) {
   if(s.audioInput==AudioInput.MICROPHONE) { foregroundTypesRequested=ServiceInfo.FOREGROUND_SERVICE_TYPE_MEDIA_PROJECTION or ServiceInfo.FOREGROUND_SERVICE_TYPE_MICROPHONE; startForeground(ID,notification(),foregroundTypesRequested) }
 ...
 private fun releaseAudio(){
  ...
  runCatching{foregroundTypesRequested=ServiceInfo.FOREGROUND_SERVICE_TYPE_MEDIA_PROJECTION;startForeground(ID,notification(),foregroundTypesRequested)}
 }
 ...
 private fun performTeardown(){
  ...
  attempt("status"){if(status.isActive)status=CaptureStatus.NEEDS_MEDIA_PROJECTION_CONSENT};attempt("local status"){if(status==CaptureStatus.NEEDS_MEDIA_PROJECTION_CONSENT)LocalStatusStore.update(LocalStatusStore.snapshot().copy(captureStatus=status))}
