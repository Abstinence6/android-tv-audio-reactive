...
class AudioReactiveService : Service() {
 ...
 override fun onStartCommand(intent:Intent?,flags:Int,startId:Int):Int {
  ...
  if(!lifecycle.whileStarting { channel(); startForeground(ID,notification(animation),if(animation) ServiceInfo.FOREGROUND_SERVICE_TYPE_DATA_SYNC else ServiceInfo.FOREGROUND_SERVICE_TYPE_MEDIA_PROJECTION) }) { admission.discardPending(); return START_NOT_STICKY }
 ...
 private fun start(data:Intent,s:AudioSettings,generation:Long){
  try {
   ...
   if(!lifecycle.acquire(acquire={createAudio(p)},release={it.stop();it.release()},assign={recorder=it})) return
   if(!createVideo(p)) return
 ...
 private fun createAudio(p:MediaProjection):AudioRecord { if(checkSelfPermission(Manifest.permission.RECORD_AUDIO)!=PackageManager.PERMISSION_GRANTED)error("audio permission"); val min=AudioRecord.getMinBufferSize(48000,AudioFormat.CHANNEL_IN_STEREO,AudioFormat.ENCODING_PCM_16BIT).coerceAtLeast(8192); return AudioRecord.Builder().setAudioFormat(AudioFormat.Builder().setEncoding(AudioFormat.ENCODING_PCM_16BIT).setSampleRate(48000).setChannelMask(AudioFormat.CHANNEL_IN_STEREO).build()).setBufferSizeInBytes(min).setAudioPlaybackCaptureConfig(AudioPlaybackCaptureConfiguration.Builder(p).addMatchingUsage(AudioAttributes.USAGE_MEDIA).addMatchingUsage(AudioAttributes.USAGE_GAME).build()).build().also{it.startRecording()} }
 ...
 private fun audioLoop(initial:AudioSettings){val samples=ShortArray(CaptureCadence.ANALYSIS_SAMPLES*2);val a=PcmAnalyzer();val r=EffectFrameRenderer(initial.captureFrame().width);val smoother=RgbFrameSmoother(initial.captureFrame().bytes);var tick=0L;val began=System.nanoTime();while(running.get()){val now=System.nanoTime();val n=recorder!!.read(samples,0,samples.size,AudioRecord.READ_BLOCKING);if(n<=0)break;val s=LiveRendererSettings.apply(RuntimeSettings.snapshot());a.configureBeatTracking(s.effectParameters.beatThreshold,now);val f=a.analyzeStereo(samples,n,s.sensitivity,s.effectParameters.beatThreshold,now);val raw=r.render(s.effect,f,s.brightness,tick++,s.effectParameters);if(!sendFrame(smoother.apply(raw,FrameSmoothingPolicy.immediateBlack(s.brightness,f.signalPresent)),f,began,tick,now,s.fps))break;sleep(now,s.fps)};stop()}
 private fun videoLoop(initial:AudioSettings){val frameSpec=initial.liveCaptureFrame();val processor=VideoFrameProcessor(frameSpec.width,frameSpec.height);val audioRenderer=AudioToFullFrameRenderer(frameSpec);val smoother=RgbFrameSmoother(frameSpec.bytes);val samples=ShortArray(CaptureCadence.ANALYSIS_SAMPLES*2);val latestSamples=ShortArray(CaptureCadence.ANALYSIS_SAMPLES*2);val analyzer=PcmAnalyzer();val silence=AudioFeatures(0f,0f,0f,0f,0f,0f,FloatArray(AudioFeatures.BAND_COUNT),false);var features:AudioFeatures?=null;var frames=0L;val began=System.nanoTime();while(running.get()){val started=System.nanoTime();val s=LiveRendererSettings.apply(RuntimeSettings.snapshot());if(AudioRecordDrainPolicy.drainLatestFullBlock(samples,latestSamples){buffer->recorder!!.read(buffer,0,buffer.size,AudioRecord.READ_NON_BLOCKING)}){analyzer.configureBeatTracking(s.effectParameters.beatThreshold,started);features=analyzer.analyzeStereo(latestSamples,latestSamples.size,s.sensitivity,s.effectParameters.beatThreshold,started)};var sent=true;when(FreshFrameDispatcher.dispatch(acquireLatest={reader!!.acquireLatestImage()},release={it.close()},render={fresh->if(processor.copyImage(fresh)){if(s.renderMode==RenderMode.AUDIO)audioRenderer.render(s.effect,features?:silence,s.brightness,frames,s.effectParameters) else processor.compose(features,s,started)}else null},send={raw->val frame=smoother.apply(raw,FrameSmoothingPolicy.immediateBlack(s,features?.signalPresent));sent=sendFrame(frame,features,began,++frames,started,s.fps)})){FreshFrameDispatcher.Result.NO_FRESH_FRAME->Unit;FreshFrameDispatcher.Result.SENT->{if(!sent)break};FreshFrameDispatcher.Result.RENDER_REJECTED->{status=CaptureStatus.VIDEO_UNAVAILABLE_OR_PROTECTED;VideoCaptureFailurePolicy.blackoutAndTerminate{router?.stop();router=null};broadcast();break}};sleep(started,s.fps)};stop()}
 private fun animationLoop(initial:AudioSettings){val frameSpec=initial.liveCaptureFrame();val renderer=AnimationFrameRenderer(frameSpec);val smoother=RgbFrameSmoother(frameSpec.bytes);var frames=0L;val began=System.nanoTime();while(running.get()){val started=System.nanoTime();val s=LiveRendererSettings.apply(RuntimeSettings.snapshot());val raw=renderer.render(s.animationEffect,s.animationColour,s.brightness,frames++,s.effectParameters);if(!sendFrame(smoother.apply(raw),null,began,frames,started,s.fps))break;sleep(started,s.fps)};stop()}
 private fun sendFrame(frame:ByteArray, features:AudioFeatures?, began:Long, frames:Long, frameStartedNanos:Long, fps:Int):Boolean = try { router?.send(frame); val now=System.nanoTime(); LocalStatusStore.recordFrameTiming(now-frameStartedNanos,CaptureCadence.periodNanos(fps)); val old=LocalStatusStore.snapshot(); LocalStatusStore.update(old.copy(frames=frames,fps=frames*1_000_000_000f/(now-began).coerceAtLeast(1),lastSendSucceeded=true,rms=features?.rms?:old.rms,peak=features?.peak?:old.peak)); MqttControlService.notifyDiagnosticChanged(); true } catch(_:Exception) { terminateRouteLost(); false }
