    ...
    fun render(effect: AnimationEffect, colour: AnimationColour, brightness: Float, tick: Long, parameters: EffectParameters): ByteArray {
        ...
        for (y in 0 until frame.height) for (x in 0 until frame.width) {
            ...
            val (hue, saturation, value) = when (effect) {
                AnimationEffect.WATER, AnimationEffect.LAKE, AnimationEffect.OCEAN -> Triple(baseHue, .78f, .18f + wave * .82f)
                ...
                AnimationEffect.CANDLE, AnimationEffect.FIREPLACE, AnimationEffect.EMBERS -> Triple(if (colour == AnimationColour.AUTO) 22f else baseHue, .94f, (.18f + sparkle * (if (effect == AnimationEffect.EMBERS) .82f else .62f)) * (1f - ny * .35f))
                AnimationEffect.FIREWORKS, AnimationEffect.FIREWORK_BURSTS, AnimationEffect.SPARKLER -> Triple(baseHue + nx * 130f, .82f, (sparkle * (1f - abs(ny - .45f) * 1.5f)).coerceIn(.03f, 1f))
