...
class OutputRoutingTest {
 @Test fun exclusiveModesHaveOnlyTwoValues(){assertEquals(listOf("HYPERION","WLED"),OutputMode.entries.map{it.name});assertEquals(OutputMode.HYPERION,AudioSettings.defaults().outputMode)}
