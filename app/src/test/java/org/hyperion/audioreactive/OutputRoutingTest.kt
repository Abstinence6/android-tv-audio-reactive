...
class OutputRoutingTest {
 ...
 @Test fun calibratedVideoAndAudioVideoRouteNativeFramesToCorrectPhysicalPackets(){
  listOf(RenderMode.VIDEO,RenderMode.VIDEO_AUDIO).forEach { renderMode ->
   DatagramSocket(0,InetAddress.getLoopbackAddress()).use { receiver ->
    ...
    val settings=AudioSettings.defaults().copy(outputMode=OutputMode.WLED,renderMode=renderMode,wledSourceZones=16)
    val frame=ByteArray(settings.captureFrame().bytes)
    for(y in 0 until 72) for(x in 0 until 128){ val at=(y*128+x)*3; val color=when { x<32 -> intArrayOf(255,0,0); x<64 -> intArrayOf(0,255,0); x<96 -> intArrayOf(0,0,255); else -> intArrayOf(255,255,255) }; frame[at]=color[0].toByte();frame[at+1]=color[1].toByte();frame[at+2]=color[2].toByte() }
