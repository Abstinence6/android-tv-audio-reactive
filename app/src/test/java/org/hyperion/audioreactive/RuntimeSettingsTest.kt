...
class RuntimeSettingsTest {
    @Test fun restoredSettingsDriveRuntime() {
        ...
        assertEquals(VideoAudioEffect.BEAT_PULSE, AudioSettings.defaults().videoAudioEffect)
    ...
    @Test fun missingVideoAudioPreferenceLoadsAndPersistsTheCanonicalBeatPulse() {
        val preferences = InMemorySharedPreferences(requiredPersistedSettings())
        val store = SharedPreferencesAudioSettingsStore(TestContext(preferences))

        RuntimeSettings.initialize(store)

        assertEquals(VideoAudioEffect.BEAT_PULSE, RuntimeSettings.snapshot().videoAudioEffect)
        RuntimeSettings.update { it.copy(sensitivity = 2f) }
        assertEquals(VideoAudioEffect.BEAT_PULSE.name, preferences.getString("video_audio_effect_v3", null))
    }

    @Test fun storedLegacyBrightnessPulsePersistsTheCanonicalBeatPulseAfterAnUpdate() {
        val preferences = InMemorySharedPreferences(requiredPersistedSettings().apply {
            put("video_audio_effect_v3", VideoAudioEffect.BRIGHTNESS_PULSE.name)
        })
        val store = SharedPreferencesAudioSettingsStore(TestContext(preferences))
    ...
    @Test fun directLegacyVideoAudioUpdateIsCanonicalizedInRuntimeAndPersistence() {
        val preferences = InMemorySharedPreferences(requiredPersistedSettings())
        val store = SharedPreferencesAudioSettingsStore(TestContext(preferences))
        RuntimeSettings.initialize(store)

        RuntimeSettings.update { it.copy(videoAudioEffect = VideoAudioEffect.BRIGHTNESS_PULSE) }
    ...
    private fun requiredPersistedSettings() = mutableMapOf<String, Any?>(
        "effect" to Effect.SPECTRUM.name,
        "brightness" to .65f,
        "sensitivity" to 1f,
        "fps" to 20,
    )
    ...
    private class InMemorySharedPreferences(private val values: MutableMap<String, Any?>) : SharedPreferences {
        override fun getAll(): MutableMap<String, *> = values
        override fun getString(key: String, defValue: String?): String? = values[key] as? String ?: defValue
        override fun getStringSet(key: String, defValues: MutableSet<String>?): MutableSet<String>? = (values[key] as? Set<*>)?.filterIsInstance<String>()?.toMutableSet() ?: defValues
        ...
        private class Editor(private val values: MutableMap<String, Any?>) : SharedPreferences.Editor {
            ...
            override fun apply() { if (clear) values.clear(); changes.forEach { (key, value) -> if (value == null) values.remove(key) else values[key] = value } }
        }
    }
}
